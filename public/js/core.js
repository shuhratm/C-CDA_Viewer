...
function loadMedicalFiles(){
	$.get('/api/files', function(data) {
		var s = '';
		if(data.length === 0) {
			s = '<p style="color: #999;">No medical records found. Please add XML files to your medical records folder.</p>';
		} else {
		...
		$('#medical-files').html('<p style="color: #cc0000;">Error loading medical records. Please check your server connection.</p>');
	});
}
